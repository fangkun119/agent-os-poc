...
AgentOS 的 AI 编程实施分两个阶段，两个阶段用不同的协作工具：

| 阶段 | 工具 | 适用场景 |
|------|------|---------|
| **主体开发阶段** | Spec-Kit | 从零开发 AgentOS 1.0 的五大核心能力，9 个 Maven 模块 |
| **增量开发阶段** | 手动提示词 + Claude Code | 扩展功能、修 bug、加 Plugin Tool 等小颗粒度增量 |

这两个阶段的边界很清晰：**Spec-Kit 适合大颗粒度 greenfield，手动提示词适合小颗粒度增量**。AgentOS 主体开发是前者，社区接力是后者，工具选择跟工作性质匹配。
...
整个 AgentOS 主体开发按 **5 个 user story** 组织，每个对应一个核心能力。
...
## 2. Spec-Kit 跟 AgentOS 的匹配度评估

写实施计划之前先回答一个根本问题：Spec-Kit 真的适合 AgentOS 项目吗？
...
| AI agent 协作 | AgentOS 本来就是用 Claude Code 做主体开发 | ✅ 完全匹配 |
| 方法论场景 | Spec-Kit 的强制流程让产出对齐需求，对开发者掌握工程方法论很有价值 | ✅ 匹配 |

**结论**：Spec-Kit 是 AgentOS 主体开发的最佳工具选择。社区在 brownfield 项目上对 Spec-Kit 有争议，但 AgentOS 是纯 greenfield，这些争议不适用。
...
- `agentos-provider`（核心能力一）
- `agentos-boot`（Spring Boot 启动模块）
...
**核心目标**：实现 Agent 的核心工作机制。即：LLM 思考是否调用工具，调用之后看结果，再决定下一步，直到给出最终响应。ReAct 循环是 AgentOS 最关键的一段代码。
...
**核心目标**：让业务方扩展 AgentOS 的能力。Plugin Tool 三档接入：
1. 零代码 `AGENT.md` 目录 + 复用 MCP（主推）
2. 轻代码自写 MCP server
3. 重代码 Java `@Tool` 注解

核心阶段做完三档基础设施 + 内置 Tool 补齐。验收对应需求文档 Demo 二（每日科技日报）的零代码 + MCP 能力面。

**涉及的 Maven 模块**：
- `agentos-tool`（补齐文件 Tool + Shell Tool、MCP Client、`SandboxChecker` 完整版、`ToolRegistry`，三合一模块）
- `agentos-core`（`AGENT.md` 的加载归 `ContextLoader`，不在 tool 模块；Agent 目录 `skills/` 软连接的 Skill 元数据注入同属 `ContextLoader`）
...
**核心目标**：把 AgentOS 的所有能力通过 REST API 对外暴露，业务系统通过 HTTP 接入。这是 AgentOS 区别于个人助手项目的关键能力。
...
看到 Claude Code 生成的代码不符合 constitution，主动让 AI agent 重读 constitution 改正。AgentOS 最容易被写错的几个点：
...
| 把 Tool 又拆成多模块 | 应该合并为一个 `agentos-tool` 模块 |
...
主体开发完成后 AgentOS 进入增量阶段。这个阶段的工作性质跟主体开发完全不同：
...
**Spec-Kit 跟 AgentOS 的契合度很高**：纯 greenfield、medium 规模（9 个模块）、需求清晰、AI agent 协作、方法论场景，每条都对得上。

**核心策略是已有文档喂给 Spec-Kit，不重写**。AgentOS 已经投入了完整的业界调研 + 需求文档 + 技术方案，这些是 Spec-Kit 的最佳输入。**关键是喂的是最新版文档**：模块是 9 个不是 11 个，constitution 要包含"Spring AI 只用一半"、"审计 day one 落库"这些新决策，否则 Spec-Kit 生成的 plan 会按旧结构走偏。
